...
PhoneLocalizer::PhoneLocalizer() :  nh(), private_nh(ros::NodeHandle("~")),
    ...
    //// Odom
    pub_odom = nh.advertise<nav_msgs::Odometry>("/"+phone_name+"/"+TOPIC_NAME_PUBLISH_ODOM, 1);
    ...
    //// Particles
    pub_particles=nh.advertise<geometry_msgs::PoseArray>("/"+phone_name+"/"+TOPIC_NAME_PUBLISH_POSEARRAY,1);
    ...
    if(use_mag){
        sub_magnetic = nh.subscribe("/"+phone_name+"/"+TOPIC_NAME_MAGNETIC, 1, &PhoneLocalizer::callback_magnetic, this);
    }
    if(use_gps){
...
PhoneLocalizer::PhoneLocalizer(std::string phone_name) : phone_name(phone_name){
    PhoneLocalizer();
}
