...
/*
    //// TODO: なし
    z_pred_cv_ =z_pred_ctrv_= z_pred_rm_=Eigen::VectorXd(5);
    s_cv_ =s_ctrv_= s_rm_=Eigen::MatrixXd(5, 5);
    k_cv_ =k_ctrv_= k_rm_=Eigen::MatrixXd(5, 5);
*//*
 ...
 * TODO: なし
    //// 全て5に変換
    r_cv_=r_ctrv_ =r_rm_= Eigen::MatrixXd(2, 2);
*/
...
double UkfEstimator::normalizeAngle(const double angle) {
...
void UkfEstimator::initialize(const Eigen::VectorXd &z, const double timestamp) {
...
*/
    ...
    //// TODO ukfのpの初期値をodom_reliabilityをtransformした値に変更
    // first_orientation_computation covariance matrix by hardcoding since no clue about initial state covrariance
    p_ctrv_ << 2.0, 0, 0, 0, 0, 0, 2.0, 0, 0, 0, 0, 0, 3, 0, 0, 0, 0, 0, 10, 0, 0, 0, 0, 0, 1;
    ...
    // if non-linear characteristics is not high, alpha should be >1.0
...
/*
    //// TODO: 全ての値をzに変換
    z_pred_cv_=z_pred_ctrv_=z_pred_rm_=z;

    x_cv_ = x_ctrv_ = x_rm_ = x_merge_=z;
...
/*
void UkfEstimator::updateModeProb(const std::vector<double>& lambda_vec)
{
...
/*
void UkfEstimator::updateYawWithHighProb()
{
...
/*
void UkfEstimator::mergeEstimationAndCovariance()
{
...
/*
void UkfEstimator::mixingProbability()
{
...
void UkfEstimator::predictionIMMUKF(const double dt) {
    /*****************************************************************************
    *  Init covariance Q if it is needed
    ****************************************************************************/
    initCovarQs(dt, x_ctrv_(3));
    /*****************************************************************************
...
/*
void UkfEstimator::findMaxZandSIMU(Eigen::VectorXd& max_det_z, Eigen::MatrixXd& max_det_s)
{
    double cv_det = s_imu_cv_.determinant();
    double ctrv_det = s_imu_ctrv_.determinant();
    double rm_det = s_imu_rm_.determinant();

    if (cv_det > ctrv_det)
    {
        if (cv_det > rm_det)
        {
            max_det_z = z_pred_imu_cv_;
            max_det_s = s_imu_cv_;
        }
        else
        {
            max_det_z = z_pred_imu_rm_;
            max_det_s = s_imu_rm_;
        }
    }
    else
    {
        if (ctrv_det > rm_det)
        {
            max_det_z = z_pred_imu_ctrv_;
            max_det_s = s_imu_ctrv_;
        }
        else
        {
            max_det_z = z_pred_imu_rm_;
...
void UkfEstimator::findMaxZandSGnss(Eigen::VectorXd& max_det_z, Eigen::MatrixXd& max_det_s)
{
    double cv_det = s_gnssraw_cv_.determinant();
    double ctrv_det = s_gnssraw_ctrv_.determinant();
    double rm_det = s_gnssraw_rm_.determinant();

    if (cv_det > ctrv_det)
    {
        if (cv_det > rm_det)
        {
            max_det_z = z_pred_gnssraw_cv_;
            max_det_s = s_cv_;
        }
        else
        {
            max_det_z = z_pred_gnssraw_rm_;
            max_det_s = s_gnssraw_rm_;
        }
    }
    else
    {
        if (ctrv_det > rm_det)
        {
            max_det_z = z_pred_gnssraw_ctrv_;
            max_det_s = s_gnssraw_ctrv_;
        }
        else
        {
            max_det_z = z_pred_gnssraw_rm_;
...
void UkfEstimator::findMaxZandSLidar(Eigen::VectorXd& max_det_z, Eigen::MatrixXd& max_det_s)
{
...
void UkfEstimator::findMaxZandS(Eigen::VectorXd& max_det_z, Eigen::MatrixXd& max_det_s)
{
/*
    double cv_det = s_cv_.determinant();
*/
...
/*
void UkfEstimator::updateEachMotionIMU(const double detection_probability, const double gate_probability, const double gating_thres,
                                         double yaw,std::vector<double>& lambda_vec)
{
    int num_meas_state = 1;
    ...
    findMaxZandSIMU(max_det_z, max_det_s);
    ...
    for (int motion_ind = 0; motion_ind < num_motion_model_; motion_ind++)
    {
        ...
        if (motion_ind == MotionModel::CV)
        {
            ...
            z_pred = z_pred_imu_cv_;
            s_pred = s_imu_cv_;
            kalman_gain = k_imu_cv_;
        }
        else if (motion_ind == MotionModel::CTRV)
        {
            ...
            s_pred = Eigen::MatrixXd(num_meas_state, num_meas_state);
            kalman_gain = Eigen::MatrixXd(num_state_, num_meas_state);
            z_pred = z_pred_imu_ctrv_;
            s_pred = s_imu_ctrv_;
            kalman_gain = k_imu_ctrv_;
        }
        else
        {
            x = x_rm_;
            ...
            s_pred = Eigen::MatrixXd(num_meas_state, num_meas_state);
            kalman_gain = Eigen::MatrixXd(num_state_, num_meas_state);
            z_pred = z_pred_imu_rm_;
            s_pred = s_imu_rm_;
            kalman_gain = k_imu_rm_;
        }
...
void UkfEstimator::updateEachMotionLidar(const double detection_probability, const double gate_probability, const double gating_thres,
                           ...
                           std::vector<double>& lambda_vec)
{
    ...
    Eigen::VectorXd max_det_z;
    Eigen::MatrixXd max_det_s;
    findMaxZandSLidar(max_det_z, max_det_s);
    double Vk = M_PI * sqrt(gating_thres * max_det_s.determinant());
...
*/
void UkfEstimator::updateEachMotion(const double detection_probability, const double gate_probability,
                                    const double gating_thres,
                                    const Eigen::VectorXd z, std::vector<double> &lambda_vec) {
    int num_meas_state = z.rows();
    // calculating association probability
    double num_meas = 1;
    double b = 2 * num_meas * (1 - detection_probability * gate_probability) / (gating_thres * detection_probability);
    ...
    int motion_ind = MotionModel::CTRV;

    Eigen::MatrixXd x(x_ctrv_.rows(), x_ctrv_.cols());
    Eigen::MatrixXd p(p_ctrv_.rows(), p_ctrv_.cols());
    Eigen::VectorXd z_pred;
    Eigen::MatrixXd s_pred;
    Eigen::MatrixXd kalman_gain;
    Eigen::VectorXd likely_meas;
...
*/
    ...
    // update x and P
    Eigen::MatrixXd updated_x(x_ctrv_.rows(), x_ctrv_.cols());
    updated_x = x + kalman_gain * diff;

    updated_x(3) = normalizeAngle(updated_x(3));

    Eigen::MatrixXd updated_p(p_ctrv_.rows(), p_ctrv_.cols());
/*
    if (num_meas != 0) {
        //// TODO s_pred内には，計測誤差がすでに考慮視されている
        updated_p = beta_zero * p + (1 - beta_zero) * (p - kalman_gain * s_pred * kalman_gain.transpose()) +
...
/*
    ...
    if (num_meas != 0) {
        lambda =
                (1 - gate_probability * detection_probability) / pow(Vk, num_meas) +
                detection_probability * pow(Vk, 1 - num_meas) * e_sum /
                (num_meas * sqrt(2 * M_PI * s_pred.determinant()));
    } else {
        lambda = (1 - gate_probability * detection_probability);
    }
...
void UkfEstimator::ctrv(const double p_x, const double p_y, const double v, const double yaw, const double yawd,
...
/*
void UkfEstimator::cv(const double p_x, const double p_y, const double v, const double yaw, const double yawd,
             const double delta_t, std::vector<double>& state)
{
...
void UkfEstimator::randomMotion(const double p_x, const double p_y, const double v, const double yaw, const double yawd,
                       const double delta_t, std::vector<double>& state)
{
    // Reference: Bayesian Environment Representation, Prediction, and Criticality Assessment for Driver Assistance
    // Systems, 2016
...
void UkfEstimator::initCovarQs(const double dt, const double yaw) {
...
void UkfEstimator::predictionMotion(const double delta_t, const int model_ind) {
    /*****************************************************************************
   *  Initialize model parameters
   ****************************************************************************/
    Eigen::MatrixXd x(x_ctrv_.rows(), 1);
    Eigen::MatrixXd p(p_ctrv_.rows(), p_ctrv_.cols());
    Eigen::MatrixXd q(p_ctrv_.rows(), p_ctrv_.cols());
    Eigen::MatrixXd x_sig_pred(x_sig_pred_ctrv_.rows(), x_sig_pred_ctrv_.cols());
...
*/
    ...
    ****************************************************************************/
    ...
    for (int i = 0; i < 2 * num_state_ + 1; i++) {  // iterate over sigma points
        ...
        if(i!=0){
            hoge+=weights_s_(i) * x_sig_pred(0,i);
        }
    }
...
/*
void UkfEstimator::updateKalmanGainIMU(const int motion_ind)
{
    ...
    Eigen::MatrixXd kalman_gain = cross_covariance * s_pred.inverse();
        ...
        if (motion_ind == MotionModel::CV)
        {
            k_imu_cv_ = kalman_gain;
        }
        else if (motion_ind == MotionModel::CTRV)
        {
            k_imu_ctrv_ = kalman_gain;
        }
        else
        {
            k_imu_rm_ = kalman_gain;
        }
}
*/

void UkfEstimator::updateKalmanGain(const int motion_ind, std::vector<int> update_state) {
    Eigen::VectorXd x(x_ctrv_.rows());
    Eigen::MatrixXd x_sig_pred(x_sig_pred_ctrv_.rows(), x_sig_pred_ctrv_.cols());
    Eigen::VectorXd z_pred;
    Eigen::MatrixXd s_pred;
    int num_meas_state = z_pred_ctrv_.rows();
    if (motion_ind == MotionModel::CV) {
/*
        x = x_cv_.col(0);
        x_sig_pred = x_sig_pred_cv_;
...
*/
    ...
    for (int i = 0; i < 2 * num_state_ + 1; i++) {
        ...
        for (int j = 0; j < num_meas_state; j++) {
            z_sig_point(j) = x_sig_pred(update_state[j], i);
        }
        Eigen::VectorXd z_diff = z_sig_point - z_pred;
        Eigen::VectorXd x_diff = x_sig_pred.col(i) - x;
...
void UkfEstimator::predictionMeasurement(const int motion_ind, std::vector<int> update_state) {
...
/*
void UkfEstimator::predictionLidarMeasurement(const int motion_ind, const int num_meas_state)
{
    Eigen::MatrixXd x_sig_pred(x_sig_pred_cv_.rows(), x_sig_pred_cv_.cols());
    Eigen::MatrixXd covariance_r(num_meas_state, num_meas_state);
...
/*
double UkfEstimator::calculateNIS(const autoware_msgs::DetectedObject& in_object, const int motion_ind)
{
...
void UkfEstimator::prediction(const double timestamp) {
