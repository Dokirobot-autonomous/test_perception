...
namespace
{
constexpr double HEIGHT_LIMIT = -1.0;  // from sensor
constexpr double CAR_LENGTH = 4.5;
constexpr double CAR_WIDTH = 1.75;
constexpr double LINE_DEPTH_TH=10;
...
cv::Mat_<int> createCostMap(const pcl::PointCloud<pcl::PointXYZ> &scan)
{
    cv::Mat_<int> cost_map_cv=cv::Mat_<int>::ones(g_cell_height,g_cell_width)*-1;
    cv::Mat_<int> cost_map_line=cv::Mat_<int>::ones(g_cell_height,g_cell_width)*-1;
    ...
    for (const auto &p : scan.points) {
        ...
        if (cost_map_cv(grid_y,grid_x)==-1){
            cost_map_cv(grid_y,grid_x)=0;
        }
        cost_map_cv(grid_y,grid_x)+=15;

        if (cost_map_cv(grid_y,grid_x)>LINE_DEPTH_TH){
            cv::line(cost_map_line,cv::Point(map_center_x/g_resolution,map_center_y/g_resolution),cv::Point(grid_x,grid_y),0);
        }

        // Max cost value is 100
        if(cost_map_cv(grid_y,grid_x)>100){
            cost_map_cv(grid_y,grid_x)=100;
        }
    }

    for(size_t y=0;y<cost_map_cv.rows;y++){
        for(size_t x=0;x<cost_map_cv.cols;x++){
            if(cost_map_cv(y,x)==-1 && cost_map_line(y,x)!=-1){
                cost_map_cv(y,x)=cost_map_line(y,x);
                cost_map_cv(y,x)=cost_map_line(y,x);
            }
        }
    }
...
//
//
//  std::vector<int> cost_map(g_cell_width * g_cell_height, -1);
//  double map_center_x = (g_cell_width / 2.0) * g_resolution - g_offset_x;
//  double map_center_y = (g_cell_height / 2.0) * g_resolution - g_offset_y;
//
//  // scan points are in sensor frame
...
//  std::vector<int> cost_map = createCostMap(scan);
    cv::Mat_<int> cost_map = createCostMap(scan);
