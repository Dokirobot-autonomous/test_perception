...
bool CNNSegmentation::init() {
    ...
    private_node_handle.param<std::string>("points_out", points_out_, "/detection/lidar_detector/points_cluster");
    ROS_INFO("[%s] points_out: %s", __APP_NAME__, points_out_.c_str());

    private_node_handle.param<std::string>("objects_out", objects_out_, "/detection/lidar_detector/objects");
    ROS_INFO("[%s] objects_out: %s", __APP_NAME__, objects_out_.c_str());
...
void CNNSegmentation::run() {
    ...
    objects_pub_ = nh_.advertise<autoware_msgs::DetectedObjectArray>(objects_out_, 1);
...
void CNNSegmentation::pubColoredPoints(const autoware_msgs::DetectedObjectArray &objects_array) {
    ...
    ostr<<objects_array.header<<std::endl;
    for (const autoware_msgs::DetectedObject &obj:objects_array.objects) {
        ostr << "Label: " << obj.label << std::endl;
        ostr << obj.pose << std::endl;
    }
    ROS_DEBUG_STREAM("Detected Objects: \n" << ostr.str());
