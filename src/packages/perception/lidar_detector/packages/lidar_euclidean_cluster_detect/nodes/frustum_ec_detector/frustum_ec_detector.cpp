...
void detection_callback(const autoware_msgs::DetectedObjectArray::ConstPtr objects_in_msg)
{
    //_start = std::chrono::system_clock::now();

    std::cout<<__FILE__<<","<<__LINE__<<std::endl;

    autoware_msgs::DetectedObjectArray objects_in=*objects_in_msg;
    ...
    for(const autoware_msgs::DetectedObject& obj_in:objects_in.objects){
        ...
        current_sensor_cloud_ptr->clear();
        ...
        pcl::fromROSMsg(obj_in.pointcloud, *current_sensor_cloud_ptr);

        _velodyne_header = obj_in.pointcloud.header;
...
//        publishCentroids(&_centroid_pub, centroids, _output_frame, _velodyne_header);
        ...
        autoware_msgs::DetectedObject obj_out;
        obj_out.header=obj_in.header;
        obj_out.label=obj_in.label;
        obj_out.score=obj_in.score;
        obj_out.color=obj_in.color;
        obj_out.valid=obj_in.valid;
        obj_out.image_frame=obj_in.image_frame;
        obj_out.x=obj_in.x;
        obj_out.y=obj_in.y;
        obj_out.width=obj_in.width;
        obj_out.height=obj_in.height;
        obj_out.angle=obj_in.angle;
        obj_out.roi_image=obj_in.roi_image;
        int i_max=0,s_max=0;
        for(int i=0;i<cloud_clusters.clusters.size();i++){
            int s=cloud_clusters.clusters[i].cloud.width;
            if (s_max<s){
                s_max=s;
                i_max=i;
            }
        }
        obj_out.space_frame=cloud_clusters.header.frame_id;
        obj_out.pose = cloud_clusters.clusters[i_max].bounding_box.pose;
        obj_out.dimensions = cloud_clusters.clusters[i_max].dimensions;
        obj_out.pointcloud = cloud_clusters.clusters[i_max].cloud;
        obj_out.pointcloud.header=_velodyne_header;
        obj_out.convex_hull = cloud_clusters.clusters[i_max].convex_hull;
        obj_out.valid = true;

        objects_out.objects.push_back(obj_out);
    }

    objects_out.header=_velodyne_header;
    _pub_detected_objects.publish(objects_out);
...
//  _pub_clusters_message = h.advertise<autoware_msgs::CloudClusterArray>("/detection/lidar_detector/cloud_clusters", 1);
    _pub_detected_objects = h.advertise<autoware_msgs::DetectedObjectArray>("/detection/frustum_detector/objects", 1);

    std::string input_topic, gridmap_topic;

    _using_sensor_cloud = false;

    if (private_nh.getParam("input_topic", input_topic))
    {
        ROS_INFO("euclidean_cluster > Setting points node to %s", input_topic.c_str());
    }
    else
    {
        ROS_INFO("euclidean_cluster > No points node received, defaulting to points_raw, you can use "
                 "_points_node:=YOUR_TOPIC");
        input_topic = "/detection/frustum_detector/objects_before_segmentation";
    }
  ...
  private_nh.param("clustering_distance", _clustering_distance, 0.2);
  ...
  ros::Subscriber sub = h.subscribe(input_topic, 1, detection_callback);
