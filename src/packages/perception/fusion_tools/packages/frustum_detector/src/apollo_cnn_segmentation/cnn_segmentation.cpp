...
#include "apollo_cnn_segmentation/cnn_segmentation.h"
...
bool CNNSegmentation::init() {
    ...
    private_node_handle.param<std::string>("objects_before_segmentation", topic_src_,
                                           "/detection/frustum_detector/objects_before_segmentation");
    ROS_INFO("[%s] objects_before_segmentation: %s", __APP_NAME__, topic_src_.c_str());
...
void CNNSegmentation::run() {
    ...
    frustum_points_sub_ = nh_.subscribe(topic_src_, 1, &CNNSegmentation::frustumPointsCallback, this);
    points_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/detection/frustum_detector/points_cluster", 1);
    objects_pub_ = nh_.advertise<autoware_msgs::DetectedObjectArray>("/detection/frustum_detector/objects", 1);

    ROS_INFO("[%s] Ready. Waiting for data...", __APP_NAME__);
}

void CNNSegmentation::frustumPointsCallback(const autoware_msgs::DetectedObjectArray::ConstPtr &msg) {
    std::chrono::system_clock::time_point start, end;
    start = std::chrono::system_clock::now();

    autoware_msgs::DetectedObjectArray objects_in = *msg;
    autoware_msgs::DetectedObjectArray objects_out;
    objects_out.header = objects_in.header;

    for (const autoware_msgs::DetectedObject &obj_in:objects_in.objects) {
        pcl::PointCloud<pcl::PointXYZI>::Ptr in_pc_ptr(new pcl::PointCloud <pcl::PointXYZI>);
        pcl::fromROSMsg(obj_in.pointcloud, *in_pc_ptr);
        message_lidar_header_ = obj_in.pointcloud.header;
        pcl::PointIndices valid_idx;
        auto &indices = valid_idx.indices;
        indices.resize(in_pc_ptr->size());
        std::iota(indices.begin(), indices.end(), 0);
        autoware_msgs::DetectedObjectArray objects_segmented;
        segment(in_pc_ptr, valid_idx, objects_segmented);
        for (const autoware_msgs::DetectedObject &obj_seg:objects_segmented.objects) {
            std::cout<<obj_seg.label<<std::endl;
            if (obj_seg.label == obj_in.label) {
                autoware_msgs::DetectedObject obj_out = obj_in;
                obj_out.space_frame = obj_seg.space_frame;
                obj_out.pose = obj_seg.pose;
                obj_out.dimensions = obj_seg.dimensions;
                obj_out.variance = obj_seg.variance;
                obj_out.velocity = obj_seg.velocity;
                obj_out.acceleration = obj_seg.acceleration;
                obj_out.pointcloud = obj_seg.pointcloud;
                obj_out.pointcloud.header = message_lidar_header_;
                obj_out.convex_hull = obj_seg.convex_hull;
                obj_out.candidate_trajectories = obj_seg.candidate_trajectories;
                obj_out.pose_reliable = obj_seg.pose_reliable;
                obj_out.velocity_reliable = obj_seg.velocity_reliable;
                obj_out.acceleration_reliable = obj_seg.acceleration_reliable;
                objects_out.objects.push_back(obj_out);
            }
        }
    }

    pubColoredPoints(objects_out);

    objects_pub_.publish(objects_out);
...
void CNNSegmentation::pubColoredPoints(const autoware_msgs::DetectedObjectArray &objects_array) {
    ...
    output_colored_cloud.header = message_lidar_header_;
