...
cv::Point3f
FrustumFusion::TransformPoint(const geometry_msgs::Point &in_point,
...
cv::Point2i
FrustumFusion::ProjectPoint(const cv::Point3f &in_point) {
...
FrustumFusion::TransformObject(const autoware_msgs::DetectedObject &in_detection,
                                         const tf::StampedTransform &in_transform) {
...
bool
FrustumFusion::IsObjectInImage(const autoware_msgs::DetectedObject &in_detection) {
...
cv::Rect FrustumFusion::ProjectDetectionToRect(const autoware_msgs::DetectedObject &in_detection) {
...
void
FrustumFusion::TransformRangeToVision(const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections,
...
void
FrustumFusion::CalculateObjectFeatures(autoware_msgs::DetectedObject &in_out_object, bool in_estimate_pose) {
...
autoware_msgs::DetectedObject FrustumFusion::MergeObjects(const autoware_msgs::DetectedObject &in_object_a,
...
double FrustumFusion::GetDistanceToObject(const autoware_msgs::DetectedObject &in_object) {
...
void FrustumFusion::CheckMinimumDimensions(autoware_msgs::DetectedObject &in_out_object) {
...
FrustumFusion::FuseRangeVisionDetections(
...
void
FrustumFusion::SyncedDetectionsCallback(
        const autoware_msgs::DetectedObjectArray::ConstPtr &in_vision_detections,
        const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections) {
...
void
FrustumFusion::VisionDetectionsCallback(
        const autoware_msgs::DetectedObjectArray::ConstPtr &in_vision_detections) {
...
void
FrustumFusion::RangeDetectionsCallback(
        const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections) {
    ROS_DEBUG("[%s]Subscribe VisionObjects", __APP_NAME__);
...
void FrustumFusion::ImageCallback(const sensor_msgs::Image::ConstPtr &in_image_msg) {
    if (!camera_info_ok_)
        return;
...
void
FrustumFusion::IntrinsicsCallback(const sensor_msgs::CameraInfo &in_message) {
...
tf::StampedTransform
FrustumFusion::FindTransform(const std::string &in_target_frame, const std::string &in_source_frame) {
...
void
FrustumFusion::InitializeROSIo(ros::NodeHandle &in_private_handle) {
    //get params
    ...
    intrinsics_subscriber_ = in_private_handle.subscribe(camera_info_src,
                                                         1,
                                                         &FrustumFusion::IntrinsicsCallback, this);
    ...
    if (!sync_topics) {
        ...
        detections_synchronizer_->registerCallback(
                boost::bind(&FrustumFusion::SyncedDetectionsCallback, this, _1, _2));
    }
