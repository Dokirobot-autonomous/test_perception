...
void ImmUkfPdaMulti::run()
{
    ...
    pub_msc_object_array_ = node_handle_.advertise<msc_msgs::ObjectArray>("/detection/msc_objects", 1);
//    pub_states_array_ = node_handle_.advertise<autoware_msgs::UkfStateArray>("/detection/states",1);
...
/*
void ImmUkfPdaMulti::callback(const autoware_msgs::DetectedObjectArray& input)
{
...
*/
    ...
    //// TODO: Add Label probablity
    msc_msgs::ObjectArray msc_objects;
    msc_objects.header=detected_objects_output.header;
    for(const auto& target:targets_){
        msc_msgs::Object obj;
        obj.header.frame_id=msc_objects.header.frame_id;
        obj.header.stamp.fromSec(target.timestamp_);
        double sum=std::accumulate(target.label_num_.begin(),target.label_num_.end(),0);
        if(target.tracking_num_<TrackingState::Stable){
            obj.existence_probability.data=(double)target.tracking_num_/(double)TrackingState::Stable;
        }
        else{
            double p=TrackingState::Lost-target.tracking_num_;
            obj.existence_probability.data=p/(double)(TrackingState::Lost-TrackingState::Stable);
        }
        for(size_t i=0;i<target.label_candidates_.size();i++){
            std_msgs::String str_msg;
            str_msg.data=target.label_candidates_[i];
            obj.label_candidates.push_back(str_msg);
            obj.label_probabilities.data.push_back((double)target.label_num_[i]/sum);
        }
        obj.mean.data.resize(target.x_merge_.rows());
        obj.covariance.data.resize(target.p_merge_.rows()*target.p_merge_.cols());
        for(size_t i=0;i<5;i++){
            obj.mean.data[i]=target.x_merge_(i,0);
            for(size_t j=0;j<5;j++){
                obj.covariance.data[i*5+j]=target.p_merge_(i,j);
            }
        }
        msc_objects.objects.push_back(obj);
    }
    pub_msc_object_array_.publish(msc_objects);


    /** For Ukf State Array**/
/*
    autoware_msgs::UkfStateArray states;
    states.header=detected_objects_output.header;
    for(const auto& target:targets_){
        std_msgs::Float64MultiArray mean,covariance;
        mean.data.resize(5);
        covariance.data.resize(25);
        for(size_t i=0;i<5;i++){
            mean.data[i]=target.x_merge_(i,0);
            for(size_t j=0;j<5;j++){
                covariance.data[i*5+j]=target.p_merge_(i,j);
            }
        }
        states.means.push_back(mean);
        states.covariances.push_back(covariance);
    }
    pub_states_array_.publish(states);
*/
                                                 ...
                                                 UKF& target)
{
    ...
    {
        //// TODO: update probability of each target label existence
        target.label_ = target.object_.label;
        auto itr=std::find(target.label_candidates_.begin(),target.label_candidates_.end(),target.object_.label);
        if(itr!=target.label_candidates_.end()){
            size_t idx=std::distance(target.label_candidates_.begin(),itr);
            target.label_num_[idx]++;
        }
        else{
            target.label_candidates_.push_back(target.object_.label);
            target.label_num_.push_back(1);
        }
    }
...
void ImmUkfPdaMulti::updateBehaviorState(const UKF& target, const bool use_sukf, autoware_msgs::DetectedObject& object)
{
    if(use_sukf)
    {
...
void ImmUkfPdaMulti::initTracker(const autoware_msgs::DetectedObjectArray& input, double timestamp)
{
    for (size_t i = 0; i < input.objects.size(); i++)
    {
        ...
        ukf.label_candidates_.push_back(input.objects[i].label);
        ukf.label_num_.push_back(1);
...
{
    //// TODO| (protbability of target existence) = tracking_num_ / TrackingState::Stable
    if (object_vec.size() > 0)
    {
        if (target.tracking_num_ < TrackingState::Stable)
        {
...
bool ImmUkfPdaMulti::probabilisticDataAssociation(const autoware_msgs::DetectedObjectArray& input, const double timestamp,
                                             std::vector<bool>& matching_vec,
