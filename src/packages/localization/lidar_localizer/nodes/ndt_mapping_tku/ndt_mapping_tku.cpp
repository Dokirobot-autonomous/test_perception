...
ros::NodeHandle* private_nh_global;

static int callback_nums=0;
...
void points_callback(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  for (int i = 0; i < (int)filtered_scan_ptr->points.size(); i++)
  {
    ...
    // scanが閾値以上の場合、skip
    int ohs_x=std::abs(scan_points[j].x);
...
//        // std::cout<<__FILE__<<","<<__LINE__<<std::]
//
//        ;
...
//          std::cout<<__FILE__<<","<<__LINE__<<std::endl;
  ...
  localizer_pose_msg.header.frame_id = "/ndt_map";
...
//  br.sendTransform(tf::StampedTransform(transform, current_scan_time, "map", "base_link"));
  ...
  std::cout << "-----------------------------------------------------------------" << std::endl;
    ...
    int entries_played;
    if(nh_global->getParam("entries_played",entries_played)){
        ROS_INFO_STREAM(entries_played<<","<<callback_nums);
    }

    private_nh_global->setParam("callback_num",callback_nums);

    int exist_kitti_player;
    if(nh_global->getParam("exist_kitti_player",exist_kitti_player)){
        if(exist_kitti_player==0){
//            ROS_INFO("hoge");
//                if(entries_played==callback_nums){
                    private_nh_global->setParam("exist_ndt_mapping",0);
                    nh_global->shutdown();
//                }
        }
    }
