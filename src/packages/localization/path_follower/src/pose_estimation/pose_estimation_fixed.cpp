...
geometry_msgs::Point previous_position;
...
void SteeringReportCallback(const geometry_msgs::TwistStamped::ConstPtr &msg) {
    actual_vel.twist.linear.x = msg->twist.linear.x;
    actual_vel.header.stamp = msg->header.stamp;
}

void ImuCallback(const sensor_msgs::Imu::ConstPtr &msg) {
    measured_orientation = tf::Quaternion(msg->orientation.x, msg->orientation.y, msg->orientation.z,
                                          msg->orientation.w);
}
...
*/
    ros::Subscriber sub1 = n.subscribe("vehicle/twist", 1, SteeringReportCallback);
    ...
    while (ros::ok()) {
        ...
        distance = sqrt(
                pow(current_position.x - initial_position.x, 2) + pow(current_position.y - initial_position.y, 2));
        ROS_DEBUG_STREAM("Initialize: " << initialize << ", Distance: " << distance << ", Twist.linear.x: "
                                        << actual_vel.twist.linear.x);
/*
    if (initialize == 0 && distance >= 5 && actual_vel.twist.linear.x > 0)
*/
        if (initial_read == 1 && initialize == 0 && distance >= 5) {
...
*/
            distance = sqrt(pow(current_position.x - previous_position.x, 2) +
                            pow(current_position.y - previous_position.y, 2));
            if (distance > 0.2) {
                actual_yaw = atan2(current_position.y - previous_position.y, current_position.x - previous_position.x);
                previous_position=current_position;
            }
            actual_rot.setEulerYPR(actual_yaw, 0, 0);
