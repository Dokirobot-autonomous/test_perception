...
double calc_summahal(PointPtr p, NDPtr nd)
{
  ...
  e = mahalanobis_on_ND(nd, q[0], q[1], q[2]);
  return e;
...
double adjust3d(PointPtr scan, int num, PosturePtr initial, int target, double *hessian_inv)
{
  ...
  double sc[3][3], sc_d[3][3][3], sc_dd[3][3][3][3];
  double *work;
  double esum = 0, gnum = 0, epsilon=0.0;
  NDPtr nd[8];
  NDMapPtr nd_map;
  int i, j, n, m, k, layer;
  double x, y, z;
...
//std::cout<<i<<","<<__FILE__<<","<<__LINE__<<std::endl;
    ...
    if (nd[j])
    {
      if (nd[j]->num > 10 && nd[j]->sign == 1)
      {
        //	double e;
        esum += calc_summand3d(&p, nd[j], pose, g, hH, qd3, dist);
        epsilon+=calc_summahal(&p, nd[j])/2.0;
        add_matrix6d(Hsumh, hH, Hsumh);
...
//std::cout<<i<<","<<__FILE__<<","<<__LINE__<<std::endl;
  ...
  for(int i=0;i<6;i++){
    for(int j=0;j<6;j++){
	std::cout<<Hinv[i][j]<<"\t";
      *(covariance+i*6+j)=2.0*epsilon*covariance_gain/(num-3)*Hinv[i][j];
    }
	std::cout<<std::endl;
  }
	std::cout<<std::endl;
